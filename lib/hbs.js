...
ExpressHbs.prototype.layoutPath = function(filename, layout) {
  ...
  [].concat(dirs).forEach(function (dir) {
    if (!layoutPath) {
      ...
      try {
        fs.statSync(layoutPath + self._options.extname);
      }
      catch(e){
...
ExpressHbs.prototype.declaredLayoutFile = function(str, filename) {
  ...
  if (matches) {
    ...
    if (this.layoutsDir && layout[0] !== '.') {
      let _layout;
      [].concat(self.layoutsDir).forEach(function (dir) {
        if (!_layout) {
          _layout = path.resolve(dir, layout);
          try {
            fs.statSync(_layout + self._options.extname);
          }
          catch(e){
            _layout = undefined;
          }
